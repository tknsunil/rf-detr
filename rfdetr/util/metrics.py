...
class MetricsPlotSink:
    ...
    def save(self):
        ...
        test_coco_eval = [
            h["test_coco_eval_bbox"] for h in self.history if "test_coco_eval_bbox" in h
        ]
        ap50_90 = np.array(
            [safe_index(x, 0) for x in test_coco_eval if x is not None],
            dtype=np.float32,
        )
        ap50 = np.array(
            [safe_index(x, 1) for x in test_coco_eval if x is not None],
            dtype=np.float32,
        )
        ar50_90 = np.array(
            [safe_index(x, 8) for x in test_coco_eval if x is not None],
            dtype=np.float32,
        )

        ema_coco_eval = [
            h["ema_test_coco_eval_bbox"]
            for h in self.history
            if "ema_test_coco_eval_bbox" in h
        ]
        ema_ap50_90 = np.array(
            [safe_index(x, 0) for x in ema_coco_eval if x is not None], dtype=np.float32
        )
        ema_ap50 = np.array(
            [safe_index(x, 1) for x in ema_coco_eval if x is not None], dtype=np.float32
        )
        ema_ar50_90 = np.array(
            [safe_index(x, 8) for x in ema_coco_eval if x is not None], dtype=np.float32
        )
        ...
        if len(epochs) > 0:
            if len(train_loss):
                axes[0][0].plot(
                    epochs, train_loss, label="Training Loss", marker="o", linestyle="-"
                )
            if len(test_loss):
                axes[0][0].plot(
                    epochs,
    ...
    def update(self, values: dict):
        ...
        if "train_loss_keypoints_l1" in values:
            self.writer.add_scalar(
                "Loss/Keypoints/L1", values["train_loss_keypoints_l1"], epoch
            )
        if "train_loss_keypoints_vis" in values:
            self.writer.add_scalar(
                "Loss/Keypoints/Visibility", values["train_loss_keypoints_vis"], epoch
            )
        if "train_loss_keypoints_oks" in values:
            self.writer.add_scalar(
                "Loss/Keypoints/OKS", values["train_loss_keypoints_oks"], epoch
            )

        # Log bbox and classification losses
        if "train_loss_ce" in values:
            self.writer.add_scalar(
                "Loss/Classification", values["train_loss_ce"], epoch
            )
...
class MetricsWandBSink:
    ...
    def __init__(
        ...
        if wandb:
            self.run = wandb.init(
                project=project, name=run, config=config, dir=output_dir
            )
