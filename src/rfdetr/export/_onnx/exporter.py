...
def onnx_simplify(
    ...
    model_opt, check_ok = onnxsim.simplify(
        sim_onnx_dir, check_n=3, input_data=input_dict, dynamic_input_shape=False
    )
    if check_ok:
    ...
    def insert_groupnorm_plugin(self):
        ...
        for node in self.graph.nodes:
            if (
                ...
                index = [
                    isinstance(inp, gs.ir.tensor.Constant) for inp in gammaNode.inputs
                ].index(True)
                gamma = np.array(
                ...
                index = [
                    isinstance(inp, gs.ir.tensor.Constant) for inp in betaNode.inputs
                ].index(True)
                beta = np.array(
